...
qsave(
  ...
  "deseq/deseq_res_vemu_treated_vs_untreated.qs"
)
...
de_vemu_cell_line_at_baseline <- de_meta_vemu %>%
  filter(concentration == 0, concentration_fbs == 5) %>%
  summarize(
    de = DESeqDataSetFromMatrix(
      de_count_mat[genes_to_keep, cur_data()$well],
      # cur_data(),
...
de_vemu_res_cell_line_at_baseline <- de_vemu_cell_line_at_baseline %>%
  mutate(
    contr = tribble(
      # de_vemu_cell_line_at_baseline$de[[1]] %>% resultsNames()
      # [1] "Intercept"                          "cell_line_COLO858_vs_A375"
      # [3] "cell_line_WM989_vs_A375"            "concentration_fbs"
      # [5] "cell_lineCOLO858.concentration_fbs" "cell_lineWM989.concentration_fbs"
      ~contrast_name, ~contrast,
      "cell_line_A375_vs_mean", c(0, -1/3, -1/3, 0, 0, 0),
      "cell_line_WM989_vs_mean", c(0, -1/3, 2/3, 0, 0, 0),
      "cell_line_COLO858_vs_mean", c(0, 2/3, -1/3, 0, 0, 0),
      # "cell_line_A375_vs_mean", c(0, -1/3, -1/3),
      # "cell_line_WM989_vs_mean", c(0, -1/3, 2/3),
      # "cell_line_COLO858_vs_mean", c(0, 2/3, -1/3),
    ) %>%
      list()
  ) %>%
  unnest(contr) %>%
...
de_vemu_res_cell_line_at_baseline_comp <- de_vemu_res_cell_line_at_baseline %>%
  ...
  inner_join(
    de_vemu_res_cell_line_at_baseline_long %>%
      select(contrast_name, ensembl_gene_id, hgnc_symbol, log2FoldChange, padj, padj_ihw),
    by = c("contrast_1" = "contrast_name"),
    relationship = "many-to-many"
  ) %>%
  inner_join(
    de_vemu_res_cell_line_at_baseline_long %>%
      select(contrast_name, ensembl_gene_id, log2FoldChange, padj, padj_ihw),
    by = c("contrast_2" = "contrast_name", "ensembl_gene_id"),
    suffix = c("_1", "_2")
  )
...
fgsea_gene_stats_grouped <- fgsea_gene_stats %>%
  group_by(comparison_type, comparison) %>%
  summarize(
...
fgsea_res_mat <- fgsea_res %>%
  transmute(comparison, pathway, signed_padj = -sign(NES)*log10(padj)) %>%
  pivot_wider(
...
topgo_gene_stats <- fgsea_gene_stats %>%
  ...
  group_by(comparison, comparison_type) %>%
  summarize(
    gene_stats = set_names(
      signed_p, ensembl_gene_id
    ) %>%
...
topgo_direction_funs <- tribble(
  ~direction, ~fun,
  "up", function(x) x > -log10(0.05),
  "down", function(x) x < log10(0.05),
  "all", function(x) abs(x) > -log10(0.05)
)
...
fgsea_res_top_clustered <- fgsea_res %>%
  ...
  cluster_df(comparison, pathway, NES) %>%
  cluster_df(pathway, comparison, NES)
...
top_go_top_clustered <- topgo_res %>%
  transmute(
    ...
    comparison_type, comparison, direction,
    term = term_unique,
...
top_go_top_res_directional <- topgo_res %>%
  ...
  mutate(
    signed_p = case_when(
      significant_up & significant_down ~ NA_real_,
