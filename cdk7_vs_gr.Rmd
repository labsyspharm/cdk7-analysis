...
deseq_res_resistant_vs_treatment_classes <- deseq_res_resistant_vs_treatment %>%
  mutate(
    ...
    combined_class = paste("resistant", resistant_class, "treatment", treatment_class, sep = "_"),
    combined_simple = case_when(
...
deseq_res_resistant_vs_treatment_classes %>%
  count(
    resistant_condition, combined_class
  ) %>%
  View()

deseq_res_resistant_vs_treatment_classes %>%
  count(
    resistant_condition, combined_simple
  ) %>%
  View()
...
p <- deseq_res_resistant_vs_treatment_classes %>%
  ...
  ggplot(
    aes(
      n, resistant_condition, fill = combined_simple
    )
  ) +
...
resistant_vs_treatment_sets <- deseq_res_resistant_vs_treatment_classes %>%
  filter(
    combined_simple %in% c("resistant_only", "opposite")
  ) %>% {
    list(
...
```{r}
selected_msigdbr <- all_gene_sets %>%
  filter(
    gs_collection == "H" |
      gs_subcollection %in% c("CP:REACTOME", "CP:KEGG_MEDICUS")
  ) %>%
  separate_wider_delim(
    gs_name,
    delim = "_",
...
fgsea_res_selected <- fgsea_res_long %>%
  semi_join(
    group_by(., updown_definition, pathway) %>%
      summarize(n_sig = sum(padj < 0.01)) %>%
      filter(n_sig > 1)
  )

fgsea_res_selected_mat <- fgsea_res_selected %>%
  filter(updown_definition == "combined") %>%
...
resistant_vs_treatment_sets_topgo_res_selected_mat <- resistant_vs_treatment_sets_topgo_res_selected %>%
  filter(updown_definition == "combined") %>%
  select(resistant_condition, term_unique, signed_p) %>%
  pivot_wider(names_from = resistant_condition, values_from = signed_p, values_fill = 0) %>%
