...
de_res <- setdiff(
  ...
  # Add main comparison Mouse YKL vs Mouse Vehicle
  bind_rows(
    extract_deseq_result(
      de, name = "GroupYKL"
    ) |>
     mutate(
      comparison = "Mouse YKL vs Mouse Vehicle"
     )
  ) |>
  power_left_join(
    gene_name_map,
  ...
  select(comparison, mouse_treatment, treatment, gene_id, gene_name, gene_biotype, species, everything())
...
de_res_between_groups <- meta$Treatment |>
  ...
  select(comparison, treatment, gene_id, gene_name, gene_biotype, species, everything())
```
